export function generateAgentsMd(task) {
    ...
    if (task.reportPath) {
        lines.push("## Report (required)", "", `When complete, write a JSON report to: \`${task.reportPath}\``, "", "Report must include:", "- execution.status: completed | failed | blocked", "- artifacts.files_modified, files_created, files_deleted", "- artifacts.test_status: passed | failed | skipped", "- cognitive_state.blockers: array of blocking issues (if any)", "");
    }
    return lines.join("\n");
}
